...
export function HeroSection(rawProps: ServicePageHeroProps) {
  ...
  const topLeftGlow =
    props.topLeftGlowClassName ??
    "from-[#4EB3E8]/10 via-transparent to-transparent dark:from-[#4EB3E8]/5";
  const titleHighlightClass = props.titleHighlightClassName ?? "text-[#4EB3E8]";

  return (
    <div className="relative overflow-hidden">
      <div
        className={`absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] ${topLeftGlow} pointer-events-none`}
      />
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            >
              {props.titleTop}
              <br />
              <span className={titleHighlightClass}>{props.titleBottom}</span>
            </h1>
          ...
          <div className="space-y-8">
            <HeroImage
              ...
              topBadge={props.topBadge}
              bottomBadge={props.bottomBadge}
            />
            {props.stats.length > 0 ? <HeroStats stats={props.stats} /> : null}
          </div>
        </div>
      </div>
    </div>
...
function HeroImage({
  ...
  src: string;
  alt: string;
  topBadge: ServicePageHeroProps["topBadge"];
  bottomBadge: ServicePageHeroProps["bottomBadge"];
}) {
  return (
    <motion.div
    ...
    >
      ...
      <FloatingBadge
        ...
        colorClass={topBadge.colorClass}
      ...
      <FloatingBadge
        ...
        title={bottomBadge.title}
        subtitle={bottomBadge.subtitle}
        colorClass={bottomBadge.colorClass}
...
function HeroStats({ stats }: { stats: Stat[] }) {
  ...
  const c0 = useAnimatedCount(stats[0]?.value ?? 0, visible, 0);
  const c1 = useAnimatedCount(stats[1]?.value ?? 0, visible, 100);
  const c2 = useAnimatedCount(stats[2]?.value ?? 0, visible, 200);
  const c3 = useAnimatedCount(stats[3]?.value ?? 0, visible, 300);
