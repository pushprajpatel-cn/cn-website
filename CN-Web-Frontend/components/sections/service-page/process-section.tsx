    ...
    <div ref={ref} className="relative overflow-hidden border-y border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-16">
        ...
        <div className="relative">
          ...
          {steps.map((step, i) => (
            <ProcessStepItem key={step.step} step={step} index={i} total={steps.length} visible={visible} />
          ))}
        </div>
      </div>
    </div>
...
function ProcessStepItem({ step, index, total, visible }: { step: ProcessStep; index: number; total: number; visible: boolean }) {
    ...
    >
      <div className="hidden md:grid md:grid-cols-[1fr_72px_1fr] w-full items-start">
        <div className={isLeft ? "pr-12" : ""}>
          {isLeft && <StepContent step={step} index={index} total={total} visible={visible} align="right" />}
        </div>
        <StepIcon3D step={step} index={index} visible={visible} />
        <div className={!isLeft ? "pl-12" : ""}>
          {!isLeft && <StepContent step={step} index={index} total={total} visible={visible} align="left" />}
        </div>
      </div>
...
function StepContent({ step, index, total, visible, align }: { step: ProcessStep; index: number; total: number; visible: boolean; align: "left" | "right" }) {
  const isRight = align === "right";
  return (
    <div className={`group flex flex-col ${isRight ? "items-end text-right" : "items-start text-left"} rounded-2xl p-6 border border-transparent hover:border-[#4EB3E8]/15 hover:bg-[#4EB3E8]/[0.02] dark:hover:bg-[#4EB3E8]/[0.04] transition-all duration-500`}>
      ...
      <p className={`mt-3 text-sm font-medium text-black/45 dark:text-white/40 leading-relaxed max-w-sm ${isRight ? "ml-auto" : ""}`}>{step.description}</p>
      <div className={`mt-4 flex items-center gap-2 w-full max-w-[200px] ${isRight ? "ml-auto" : ""}`}>
        ...
        <span className="text-[10px] font-semibold text-black/20 dark:text-white/15 tabular-nums">{index + 1}/{total}</span>
      </div>
    </div>
  );
}

function StepIcon3D({ step, index, visible }: { step: ProcessStep; index: number; visible: boolean }) {
  const StepIcon = step.icon;
  return (
    <div className="flex justify-center relative z-10">
      <div className="group relative">
        <motion.div
          ...
          transition={{ duration: 2, delay: 0.5 + index * 0.2, repeat: Infinity, repeatDelay: 3 }}
        />
